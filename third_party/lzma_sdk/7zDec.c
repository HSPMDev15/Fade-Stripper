...
static SRes SzDecodePpmd (const Byte* props,
unsigned propsSize,
UInt64 inSize,
ILookInStream* inStream,
Byte* outBuffer,
...
#define IS_BCJ2(c) \
    ((c)->MethodID == k_BCJ2 && (c)->NumInStreams == 4 && (c)->NumOutStreams == 1)
...
static SRes CheckSupportedFolder (const CSzFolder* f) {
    ...
    if (f->NumCoders == 2) {
        ...
        if (
        /* c->MethodID > (UInt32)0xFFFFFFFF || */
        c->NumInStreams != 1 || c->NumOutStreams != 1 || f->NumPackStreams != 1 ||
        f->PackStreams[0] != 0 || f->NumBindPairs != 1 ||
...
Byte* tempBuf[]) {
    ...
    for (ci = 0; ci < folder->NumCoders; ci++) {
        ...
        if (IS_MAIN_METHOD ((UInt32)coder->MethodID)) {
            ...
            res = Bcj2_Decode (tempBuf3, tempSize3, tempBuf[0], tempSizes[0],
            tempBuf[1], tempSizes[1], tempBuf[2], tempSizes[2], outBuffer, outSize);
            RINOK (res)
        } else {
            if (ci != 1)
                return SZ_ERROR_UNSUPPORTED;
...
ISzAlloc* allocMain) {
    ...
    sdSizes.Size = p->FoSizesOffsets[folderIndex + 1] - p->FoSizesOffsets[folderIndex];
