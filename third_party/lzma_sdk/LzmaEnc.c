...
#define kLenNumSymbolsTotal \
    (kLenNumLowSymbols + kLenNumMidSymbols + kLenNumHighSymbols)
...
void LzmaEnc_SaveState (CLzmaEncHandle pp) {
    ...
    for (i = 0; i < kNumLenToPosStates; i++)
        memcpy (dest->posSlotEncoder[i], p->posSlotEncoder[i],
...
SRes LzmaEnc_SetProps (CLzmaEncHandle pp, const CLzmaEncProps* props2) {
    ...
    props.dictSize > ((UInt32)1 << kDicLogSizeMaxCompress) ||
    props.dictSize > ((UInt32)1 << 30))
...
static const int kShortRepNextStates[kNumStates] = { 9, 9, 9, 9, 9, 9, 9, 11,
    11, 11, 11, 11 };
...
static void LitEnc_EncodeMatched (CRangeEnc* p, CLzmaProb* probs, UInt32 symbol, UInt32 matchByte) {
    ...
    do {
        matchByte <<= 1;
        RangeEnc_EncodeBit (p,
        probs + (offs + (matchByte & offs) + (symbol >> 8)), (symbol >> 7) & 1);
        symbol <<= 1;
...
static UInt32
LitEnc_GetPriceMatched (const CLzmaProb* probs, UInt32 symbol, UInt32 matchByte, UInt32* ProbPrices) {
    UInt32 price = 0;
    UInt32 offs  = 0x100;
    symbol |= 0x100;
    do {
...
static UInt32
RcTree_GetPrice (const CLzmaProb* probs, int numBitLevels, UInt32 symbol, UInt32* ProbPrices) {
    UInt32 price = 0;
...
static UInt32
RcTree_ReverseGetPrice (const CLzmaProb* probs, int numBitLevels, UInt32 symbol, UInt32* ProbPrices) {
    UInt32 price = 0;
    UInt32 m     = 1;
    int i;
...
static void LenEnc_Encode (CLenEnc* p, CRangeEnc* rc, UInt32 symbol, UInt32 posState) {
    if (symbol < kLenNumLowSymbols) {
        ...
        if (symbol < kLenNumLowSymbols + kLenNumMidSymbols) {
            ...
            RcTree_Encode (rc, p->mid + (posState << kLenNumMidBits),
            kLenNumMidBits, symbol - kLenNumLowSymbols);
        } else {
...
static void
LenEnc_SetPrices (CLenEnc* p, UInt32 posState, UInt32 numSymbols, UInt32* prices, UInt32* ProbPrices) {
    ...
    for (i = 0; i < kLenNumLowSymbols; i++) {
        if (i >= numSymbols)
            return;
        prices[i] = a0 +
        RcTree_GetPrice (p->low + (posState << kLenNumLowBits), kLenNumLowBits, i, ProbPrices);
    }
    for (; i < kLenNumLowSymbols + kLenNumMidSymbols; i++) {
        if (i >= numSymbols)
            return;
        prices[i] = b0 +
        RcTree_GetPrice (p->mid + (posState << kLenNumMidBits), kLenNumMidBits,
        i - kLenNumLowSymbols, ProbPrices);
    }
    for (; i < numSymbols; i++)
        prices[i] = b1 +
        RcTree_GetPrice (p->high, kLenNumHighBits,
...
static UInt32 GetOptimum (CLzmaEnc* p, UInt32 position, UInt32* backRes) {
    ...
    for (;;) {
        ...
        {
            const CLzmaProb* probs = LIT_PROBS (position, *(data - 1));
            curAnd1Price += (!IsCharState (state) ?
            LitEnc_GetPriceMatched (probs, curByte, matchByte, p->ProbPrices) :
            LitEnc_GetPrice (probs, curByte, p->ProbPrices));
        }
        ...
        startLen = 2; /* speed optimization */
        {
            ...
            for (repIndex = 0; repIndex < LZMA_NUM_REPS; repIndex++) {
                ...
                if (data[0] != data2[0] || data[1] != data2[1])
                    continue;
                for (lenTest = 2;
                     lenTest < numAvail && data[lenTest] == data2[lenTest]; lenTest++)
                    ;
                ...
                /* if (_maxMode) */
                {
                    ...
                    if (lenTest2 >= 2) {
                        ...
                        UInt32 curAndLenCharPrice = price +
                        p->repLenEnc.prices[posState][lenTest - 2] +
...
static void WriteEndMarker (CLzmaEnc* p, UInt32 posState) {
    ...
    kNumPosSlotBits, (1 << kNumPosSlotBits) - 1);
    RangeEnc_EncodeDirectBits (
    &p->rc, (((UInt32)1 << 30) - 1) >> kNumAlignBits, 30 - kNumAlignBits);
...
static void FillDistancesPrices (CLzmaEnc* p) {
    ...
    for (i = kStartPosModelIndex; i < kNumFullDistances; i++) {
        ...
        tempPrices[i]     = RcTree_ReverseGetPrice (
            p->posEncoders + base - posSlot - 1, footerBits, i - base, p->ProbPrices);
    }
        ...
        for (;;) {
            ...
            if (len == 1 && pos == (UInt32)-1) {
                ...
                if (IsCharState (p->state))
                    ...
                    &p->rc, probs, curByte, *(data - p->reps[0] - 1));
                p->state = kLiteralNextStates[p->state];
            } else {
                RangeEnc_EncodeBit (&p->rc, &p->isMatch[p->state][posState], 1);
                if (pos < LZMA_NUM_REPS) {
                    ...
                    if (pos == 0) {
                        ...
                        RangeEnc_EncodeBit (&p->rc,
                        &p->isRep0Long[p->state][posState], ((len == 1) ? 0 : 1));
                    } else {
                    ...
                    if (posSlot >= kStartPosModelIndex) {
                        ...
                        if (posSlot < kEndPosModelIndex)
                            ...
                            p->posEncoders + base - posSlot - 1, footerBits, posReduced);
                        else {
                            RangeEnc_EncodeDirectBits (&p->rc,
                            posReduced >> kNumAlignBits, footerBits - kNumAlignBits);
                            RcTree_ReverseEncode (&p->rc, p->posAlignEncoder,
                            kNumAlignBits, posReduced & kAlignMask);
                            p->alignPriceCount++;
                        }
                    }
                    p->reps[3] = p->reps[2];
...
static SRes LzmaEnc_Alloc (CLzmaEnc* p, UInt32 keepWindowSize, ISzAlloc* alloc, ISzAlloc* allocBig) {
...

    {
        ...
        if (p->litProbs == 0 || p->saveState.litProbs == 0 || p->lclp != lclp) {
            LzmaEnc_FreeLits (p, alloc);
            p->litProbs =
            (CLzmaProb*)alloc->Alloc (alloc, (0x300 << lclp) * sizeof (CLzmaProb));
...
static SRes
LzmaEnc_AllocAndInit (CLzmaEnc* p, UInt32 keepWindowSize, ISzAlloc* alloc, ISzAlloc* allocBig) {
    UInt32 i;
