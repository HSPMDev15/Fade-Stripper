...
ISzAlloc* alloc) {
    ...
    for (fo = 0; fo < numFolders; fo++) {
        ...
        for (ci = 0; ci < numCoders; ci++) {
            ...
            if ((mainByte & 0x10) != 0) {
                ...
                if (coderInStreams > NUM_CODER_STREAMS_MAX || coderOutStreams > NUM_CODER_STREAMS_MAX)
                    return SZ_ERROR_UNSUPPORTED;
            }
...

        {
            ...
            if (numOutStreams != 1 || numInStreams != 1) {
                ...
                if (numInStreams > k_InStreamUsed_MAX || numOutStreams > k_OutStreamUsed_MAX)
                    return SZ_ERROR_UNSUPPORTED;
    ...
    // MY_ALLOC(UInt64, p->CoderUnpackSizes, (size_t)numCodersOutStreams, alloc);
    {
        ...
        */
        ...
        /*
        const Byte *data = SzReadNumbers(sd.Data, sd.Data + sd.Size,
        p->CoderUnpackSizes, numCodersOutStreams); if (data == NULL) return
        SZ_ERROR_ARCHIVE; sd.Size = sd.Data + sd.Size - data; sd.Data = data;
        */
    }
...
ISzAlloc* allocTemp) {
    ...
    for (fo = 0; fo < p->NumFolders; fo++) {
        ...
        RINOK (SzAr_DecodeFolder (
        p, fo, inStream, dataStartPos, tempBuf->data, tempBuf->size, allocTemp));
...
static MY_NO_INLINE SRes ReadTime (CSzBitUi64s* p,
UInt32 num,
CSzData* sd2,
const CBuf* tempBufs,
UInt32 numTempBufs,
...
ISzAlloc* allocTemp) {
    ...
    if (type == k7zIdAdditionalStreamsInfo) {
        ...
        res            = SzReadAndDecodePackedStreams (inStream, sd, tempBufs,
                   NUM_ADDITIONAL_STREAMS_MAX, p->startPosAfterHeader, &tempAr, allocTemp);
        ...
        for (i = 0; i < numFiles; i++, mask >>= 1) {
            ...
            if (indexInFolder >= curNumSubStreams) {
                ...
                UInt64 startFolderUnpackPos;
                {
                    ...
                    UInt32 numOutStreams = mix >> 8;
                    UInt32 si;
                    p->db.FoSizesOffsets[folderIndex] =
                    sdCodersUnpSizes.Data - p->db.UnpackSizesData;
                    for (si = 0; si < numOutStreams; si++) {
                        UInt64 curSize;
                        RINOK (ReadNumber (&sdCodersUnpSizes, &curSize));
...
static SRes
SzReadHeader (CSzArEx* p, CSzData* sd, ILookInStream* inStream, ISzAlloc* allocMain, ISzAlloc* allocTemp) {
    ...
    // &emptyFileVector,
    // &lwtVector,
    inStream, tempBufs, &numTempBufs, allocMain, allocTemp);
...
static SRes
SzArEx_Open2 (CSzArEx* p, ILookInStream* inStream, ISzAlloc* allocMain, ISzAlloc* allocTemp) {
...
ISzAlloc* allocTemp) {
    ...
    *outSizeProcessed  = 0;
        ...
        *tempBuf = 0;
        ...
        if (res == SZ_OK) {
            *outBufferSize = unpackSize;
            ...
            if (res == SZ_OK) {
                res = SzAr_DecodeFolder (&p->db, folderIndex, inStream,
                p->dataPos, *tempBuf, unpackSize, allocTemp);
                if (res == SZ_OK) {
    ...
    if (res == SZ_OK) {
        ...
        *offset =
        (size_t)(unpackPos - p->UnpackPositions[p->FolderStartFileIndex[folderIndex]]);
        *outSizeProcessed = (size_t)(p->UnpackPositions[fileIndex + 1] - unpackPos);
        if (*offset + *outSizeProcessed > *outBufferSize)
            return SZ_ERROR_FAIL;
        if (SzBitWithVals_Check (&p->CRCs, fileIndex) &&
        CrcCalc (*tempBuf + *offset, *outSizeProcessed) != p->CRCs.Vals[fileIndex])
            res = SZ_ERROR_CRC;
    }
    return res;
}
