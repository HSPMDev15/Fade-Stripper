...
static SRes BraState_Code (void* pp,
Byte* dest,
SizeT* destLen,
const Byte* src,
SizeT* srcLen,
...
int* wasFinished) {
    ...
    *wasFinished      = 0;
    while (destLenOrig > 0) {
        ...
        case XZ_ID_X86:
            p->bufConv =
            x86_Convert (p->buf, p->bufTotal, p->ip, &p->x86State, p->encodeMode);
            break;
            CASE_BRA_CONV (PPC)
            CASE_BRA_CONV (IA64)
            CASE_BRA_CONV (ARM)
...
SRes BraState_SetFromMethod (IStateCoder* p, UInt64 id, int encodeMode, ISzAlloc* alloc) {
    CBraState* decoder;
    if (id != XZ_ID_Delta && id != XZ_ID_X86 && id != XZ_ID_PPC &&
    id != XZ_ID_IA64 && id != XZ_ID_ARM && id != XZ_ID_ARMT && id != XZ_ID_SPARC)
        return SZ_ERROR_UNSUPPORTED;
    p->p    = 0;
...
static SRes SbState_Code (void* pp,
Byte* dest,
SizeT* destLen,
const Byte* src,
SizeT* srcLen,
...
static SRes Lzma2State_Code (void* pp,
Byte* dest,
SizeT* destLen,
const Byte* src,
SizeT* srcLen,
...
int* wasFinished) {
    ...
    SRes res = Lzma2Dec_DecodeToBuf ((CLzma2Dec*)pp, dest, destLen, src, srcLen,
    (ELzmaFinishMode)finishMode, &status);
    srcWasFinished = srcWasFinished;
...
SRes MixCoder_Code (CMixCoder* p,
Byte* dest,
SizeT* destLen,
const Byte* src,
SizeT* srcLen,
    ...
    for (;;) {
        ...
        */
        ...
        for (i = 0; i < p->numCoders; i++) {
            ...
            res = coder->Code (coder->p, destCur, &destLenCur, srcCur,
            &srcLenCur, srcFinishedCur, finishMode, &encodingWasFinished);
    ...
    *status           = CODER_STATUS_NOT_SPECIFIED;
    for (;;) {
        ...
        if (p->state == XZ_STATE_BLOCK) {
            ...
            res = MixCoder_Code (&p->decoder, dest, &destLen2, src, &srcLen2,
            False, finishMode, status);
            ...
            if (*status == CODER_STATUS_FINISHED_WITH_MARK) {
                Byte temp[32];
                unsigned num = Xz_WriteVarInt (temp,
