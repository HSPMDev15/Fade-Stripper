...
static SRes
Xz_ReadBackward (CXzStream* p, ILookInStream* stream, Int64* startOffset, ISzAlloc* alloc) {
    UInt64 indexSize;
...
#define TEMP_BUF_SIZE (1 << 10)
    ...
    *startOffset = -(Int64)(indexSize + XZ_STREAM_FOOTER_SIZE);
...

    {
        ...
        if (totalSize == XZ_SIZE_OVERFLOW || sum >= ((UInt64)1 << 63) ||
...
ISzAlloc* alloc) {
    ...
    *startOffset = endOffset;
    for (;;) {
        ...
        RINOK (stream->Seek (stream, startOffset, SZ_SEEK_SET));
        if (progress &&
        progress->Progress (progress, endOffset - *startOffset, (UInt64)(Int64)-1) != SZ_OK)
