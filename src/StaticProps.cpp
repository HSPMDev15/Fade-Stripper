...
PatchResult patchBlob (std::vector<uint8_t>& blob, uint16_t version, const LogFn& log) {
    ...
    if (!need (4)) {
        res.ok    = false;
        res.error = "truncated in dictCount";
        return res;
    }
    ...
    if (!need (4)) {
        res.ok    = false;
        res.error = "truncated in leafCount";
        return res;
    }
    ...
    if (!need (4)) {
        res.ok    = false;
        res.error = "truncated in propCount";
        return res;
    }
    ...
    version, isV7Star ? " (v7* layout TF2SDK branch)" : "", stride, propCount)
         .c_str ());
    ...
    for (uint32_t i = 0; i < propCount; ++i) {
        ...
        (fadeMax == FADE_NEVER_MAX) && !hasFadeFlag;
        if (alreadyOk) {
            ++res.alreadyOk;
            continue;
        }
...
PatchResult patchStaticPropFades (BSP& bsp, const LogFn& log) {
    ...
    lumps, [] (const GameLump& gl) { return gl.id == GAMELUMP_STATIC_PROPS; });
    ...
    it->version, (it->flags & GAMELUMPFLAG_COMPRESSED) ? "yes" : "no")
         .c_str ());
