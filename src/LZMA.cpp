...
unsigned char* LZMA_Compress(const unsigned char* pInput,
    ...
    *pOutputSize = 0;
    ...
    if (!enc) { free(pOut); return nullptr; }
...
// The encoder writes after the slot reserved for the Valve header
    ...
    const SRes res = LzmaEnc_Encode(enc, &outStream.vt, &inStream.vt,
                                    nullptr, &g_Alloc, &g_Alloc);
    LzmaEnc_Destroy(enc, &g_Alloc, &g_Alloc);

    if (res != SZ_OK || outStream.overflow) { free(pOut); return nullptr; }
...
// Build Valve header over the reserved slot
    ...
    // Displace payload 
    memmove(pOut + sizeof(lzma_header_t),
            pOut + sizeof(lzma_header_t) + 13,
            compSz - 13);
...
bool LZMA_Uncompress(const unsigned char* pInBuffer,
    ...
    *pOutSize    = 0;
    ...
    if (!pOut) { LzmaDec_Free(&state, &g_Alloc); return false; }
    ...
    const SRes result = LzmaDecode(
        reinterpret_cast<Byte*>(pOut), &outProcessed,
        reinterpret_cast<const Byte*>(pInBuffer + sizeof(lzma_header_t)), &inProcessed,
        hdr->properties, LZMA_PROPS_SIZE,
        LZMA_FINISH_END, &status, &g_Alloc);

    LzmaDec_Free(&state, &g_Alloc);

    if (result != SZ_OK || hdr->actualSize != outProcessed) { free(pOut); return false; }
...
bool LZMA_IsCompressed(const unsigned char* pInput, size_t inputSize) {
    if (!pInput || inputSize < sizeof(lzma_header_t)) return false;
    uint32_t id;
...
unsigned int LZMA_GetActualSize(const unsigned char* pInput, size_t inputSize) {
    if (!LZMA_IsCompressed(pInput, inputSize)) return 0;
