...
bool BSP::load(std::string_view path) {
    ...
    if (rawFile_.size() < sizeof(BSPHeader_t)) return false;

    memcpy(&header_, rawFile_.data(), sizeof(BSPHeader_t));

    if (static_cast<uint32_t>(header_.ident) != IDBSPHEADER) return false;
    if (header_.m_nVersion < MINBSPVERSION ||
        header_.m_nVersion > BSPVERSION)    return false;

    parseGameLumps();
    return true;
}

BSP::BSP(std::string_view path) : path_{path} { loaded_ = load(path); }

void BSP::parseGameLumps() {
    const lump_t& gl = header_.lumps[LUMP_GAME_LUMP];
    if (gl.filelen <= 0 || gl.fileofs <= 0) return;
    if (static_cast<size_t>(gl.fileofs + gl.filelen) > rawFile_.size()) return;

    const uint8_t* base = rawFile_.data() + gl.fileofs;
    const int      len  = gl.filelen;
    if (len < static_cast<int>(sizeof(dgamelumpheader_t))) return;

    dgamelumpheader_t hdr;
    memcpy(&hdr, base, sizeof(hdr));
    if (hdr.lumpCount <= 0 || hdr.lumpCount > 64) return;

    const size_t dirSize = sizeof(dgamelumpheader_t) +
                           static_cast<size_t>(hdr.lumpCount) * sizeof(dgamelump_t);
    if (dirSize > static_cast<size_t>(len)) return;
    ...
    for (int i = 0; i < hdr.lumpCount; ++i) {
        ...
        if (compressed) {
            ...
            if (i + 1 < hdr.lumpCount) nextOfs = entries[i + 1].fileofs;
        ...
        if (diskLen <= 0 ||
            static_cast<size_t>(e.fileofs + diskLen) > rawFile_.size()) {
            gameLumps_.push_back(std::move(gl2));
            continue;
        }
...
bool BSP::bake(std::string_view outputPath) const {
    ...
    const bool anyCompressed = std::ranges::any_of(gameLumps_, [](const GameLump& gl) {
        return (gl.flags & GAMELUMPFLAG_COMPRESSED) != 0;
    });
...
bool BSP::bakeFast(const std::string& dest) const {
    ...
    for (const GameLump& gl : gameLumps_) {
        if (gl.fileofs <= 0 || gl.data.empty()) continue;
...
bool BSP::bakeFullRewrite(const std::string& dest) const {
    ...
    for (int i = 0; i < HEADER_LUMPS; ++i) {
        if (i == LUMP_MAP_FLAGS || i == LUMP_PAKFILE) continue;
    ...
    for (int i : rest) order.push_back(i);
    ...
    for (const int idx : order) {
        ...
        if (idx == LUMP_GAME_LUMP) {
            ...
            struct GLBlob { std::vector<uint8_t> payload; bool wasCompressed; };
            ...
            for (const GameLump& gl : gameLumps_) {
                ...
                if (b.wasCompressed) {
                    ...
                    if (c) { b.payload.assign(c, c + sz); free(c); }
                    else   { b.payload = gl.data; b.wasCompressed = false; }
                } else {
                    b.payload = gl.data;
                }
            ...
            const int    cnt      = static_cast<int>(gameLumps_.size()) + (anyCompressed ? 1 : 0);
            ...
            dgamelumpheader_t glHdr{}; glHdr.lumpCount = cnt;
            w.write(glHdr);
            ...
            for (size_t i = 0; i < gameLumps_.size(); ++i) {
                ...
                dgamelump_t entry{};
                entry.id      = gl.id;
                entry.flags   = b.wasCompressed ? gl.flags : (gl.flags & ~GAMELUMPFLAG_COMPRESSED);
            ...
            if (anyCompressed) {
                dgamelump_t dummy{}; dummy.fileofs = static_cast<int>(glCurOfs);
                w.write(dummy);
            }
            ...
            if (l.filelen <= 0 || l.fileofs <= 0) continue;
            if (static_cast<size_t>(l.fileofs + l.filelen) > rawFile_.size()) continue;
